...
function boxMinimized() {
    // Calculate the total width needed for all visible chat boxes
    var _boxDefaultWidth = 300; // Each chat box is 300px wide
    var _sidebarWidth = parseInt($('#sidebar').css('width').replace('px', ''), 10) || 300;
    var _windowWidth = $(window).width();

    // Count visible (non-minimized, non-hidden) chat boxes
    var visibleBoxCount = $('#main-chat .boxs .chat-single-box:not(.minimized):not(.hidden)').length;

    // Calculate total width needed: sidebar + (chat boxes * 315px each including margin)
    var _totalWidthNeeded = _sidebarWidth + (visibleBoxCount * 315) + 50; // 50px buffer

    var _hasMinimized = $('#main-chat .boxs .chat-single-box.minimized').length > 0;

    // If window is wide enough to show all boxes
    if (_windowWidth > _totalWidthNeeded) {
        if (!_hasMinimized) {
            if (_windowWidth < 768) {
        ...
        // Restore a hidden chat from minimized list
        ...
        // Window too narrow - need to minimize some boxes
        if (!_hasMinimized) {
...
$(function () {
    ...
    if (waveEffect == 'true') {
        ...
        $('#sidebar .userlist-box').each(function (index) {
...
$(document).on('click', '#main-chat .chat-single-box .mini', function () {
    ...
    var parent = $(this).parents('.chat-single-box');
...
$(document).on('click', '#main-chat .chat-single-box .chat-close', function () {
...
/*Click on username*/
$(document).on('click', '#main-chat #sidebar .userlist-box', function (e) {
    e.preventDefault();
    e.stopPropagation();
    ...
    // Check if chat box with this ID already exists
