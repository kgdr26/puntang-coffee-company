...
function initialTooltip() {
    ...
    //tooltip - Bootstrap 5: data-bs-toggle
    $('[data-bs-toggle="tooltip"]').tooltip({ delay: 50 });
    $('[data-bs-toggle="tooltip"]').tooltip({ delay: 50 });
}
...
$(document).on('click', '#main-chat .chat-single-box .smile-ico', function (e) {
    ...
    var _parent = $(this).parents('.icons');
    ...
    if (_parent.hasClass('show')) {
        ...
        var _source = _parent.data('source');
...
$(document).on('click', '#main-chat .preview-image', function () {
    ...
    var preview = '<div class="preview-overlay"><div class="preview-placeholder"><img class="preview-image" src="{0}"/><div class="preview-caption">{1}</div></div></div>';
...
$(document).on('click', '#main-chat .chat-single-box .stickers .tab-content li', function () {
    ...
    var _sendMsg = $(this).parents('.chat-footer').find('.send-message div');
    ...
    if ($(this).parents('.chat-footer').find('.send-message div').html() == '<span class="placeholder">{0}</span>'.format(_sendMsg.data('placeholder'))) {
        ...
        var _str = _sendMsg.html();
        //_sendMsg.html(_str + ' ' + '<div class="send-sticker" style="background-image:url({0})"></div>'.format(_src));
        _sendMsg.html(_str + ' ' + _img);
    }
});

// Bootstrap 5: .media replaced with .d-flex, .media-body with .flex-grow-1, .media-right with .flex-shrink-0.ms-3
$(document).on('click', '#main-chat #paper-btn', function (e) {
        ...
        _box_message.append('<div class="message out no-avatar d-flex">' +
            '<div class="body flex-grow-1 text-right p-l-50"><div class="content msg-reply f-12 bg-primary d-inline-block">'+ text +'</div><div class="seen"><i class="feather icon-clock f-12 m-r-5 txt-muted d-inline-block"></i><span><p class="d-inline-block">a few seconds ago </p></span><div class="clear"></div> </div></div>' +
            ' <div class="sender flex-shrink-0 ms-3 friend-box"><a href="javascript:void(0);" title="Me"><img src="../assets/images/avatar-1.jpg" class=" img-chat-profile" alt="Me"></a> </div>' +
            '</div>');
